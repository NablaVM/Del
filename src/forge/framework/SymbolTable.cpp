...
namespace FORGE
{
    ...
    bool SymbolTable::is_existing_symbol_of_type(std::string symbol, PrimitiveTypes type)
    {
        if(is_locked) { report_lock(); }
    ...
    PrimitiveTypes SymbolTable::get_value_type(std::string symbol)
    {
        if(is_locked) { report_lock(); }
    ...
    void SymbolTable::add_return_type_to_current_context(PrimitiveTypes type)
    {
    ...
    PrimitiveTypes SymbolTable::get_return_type_of_context(std::string context)
    {
        if(is_locked) { report_lock(); }
        ...
        return PrimitiveTypes::UNDEFINED; // For the compiler
    }
